...
- **Capture** (JPEG-only): at connect the bridge sets `StillImageStoreDestination = HostPC` (**not** HostPC+MemoryCard: a body with no card refuses to fire and only says so through `OnWarning` = `CrWarning_CautionDisplay` `0x0002008C`), calls `SetSaveInfo` to a temp dir, and enables live view. The setup writes wait for the first `OnPropertyChanged` — `OnConnected` only means the transport is up, and writing during the SDK's initial property sync kills the session (`CrError_Adaptor_GetInfo` `0x8702`). `sn_capture` sends `Release` Down → 35 ms → Up, waits for `OnCompleteDownload`, reads the saved JPEG and deletes it. Live view uses `GetLiveViewImageInfo` + `GetLiveViewImage`; "not ready" maps to `SdkError(0x0000_A102)`.
...
- **Camera requirement**: the body must be in **PC Remote** USB connection mode to enumerate. A body that sleeps or is left holding a session re-enumerates outside PC Remote (HID class) and disappears from `list_devices`.
- **Sessions must be closed**: if the process is killed rather than stopped (SIGKILL, crash, a debugger detaching), the body keeps believing a PC Remote session is open and answers `CrError_Connect_TimeOut` (`0x8208`) to every later `Connect` until it is power-cycled. `sn_connect` reports the `CrError` (not just NULL) so `connect_error` can map that case to a `CameraError::Backend` telling the user to power-cycle, instead of a misleading "device not found".
...
- **On/off properties** come as a `Toggle` widget on some drivers and as a two-choice `On`/`Off` radio on others; both are exposed as our `Boolean` kind (`on_off_choice`), and `set_parameter` translates the client's `"true"`/`"false"` back to the camera's own choice.
- **Errors keep libgphoto2's message** (`CameraError::Backend`, e.g. "Could not claim the USB device", "Camera is busy") instead of collapsing to `SdkError(0)`.
- **Parameter curation** (in `walk_widget` + `get_parameters`), mirroring the Canon backend's intent:
  - ISO is split into an `IsoAuto` boolean + an `Iso` selector; `Iso` is `disabled` when auto is on. Two families: Canon folds an "Auto" entry into the ISO list (detected as the only non-numeric choice — `auto_iso_choice`), Nikon keeps the list concrete and exposes a separate `autoiso` widget. When the toggle is a separate widget, `reconcile_iso_auto` disables the ISO selector after the whole tree is walked.
  - Duplicate parameter types are dropped, keeping the first (Nikon exposes both `focusmode` and `focusmode2`).
  - Driving `manualfocusdrive` requires the body to be in live view, so `set_parameter` takes a preview frame first (`capture_preview`) before a `Focus` write. A body can advertise the widget and still be unable to execute it — libgphoto2 builds it from the PTP ops the camera *claims* to support, and the Nikon Z5 II answers "not in live view" (a bare `GP_ERROR`) to every drive command even while streaming. Nothing in the config tree distinguishes it up front, so `get_parameters` asks the camera: `focus_drive_works` drives the motor by one step (the smallest possible move, invisible in the preview) the first time the parameter would be exposed, caches the verdict per device in `focus_drive`, and hides `Focus` when the body refuses. Only range-widget bodies (Nikon) are probed — a Canon radio has no neutral step to send, so those are taken at their word and `drive_focus` records a refusal if one ever comes. Only an outright refusal counts (`ErrorKind::Other` / `NotSupported`); "focus at limit" or a busy error mean the motor works.
