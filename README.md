...
6. Using libgphoto2 — Fuji and many other PTP/USB cameras (older Nikon bodies too). Bodies covered by a vendor SDK (Canon, Nikon Z series 2, Sony) are handled by that SDK instead, so they never appear twice.
...
8. Using the Sony Camera Remote SDK (α / ILCE / ILME / ZV / DSC bodies). **The camera must be in PC Remote mode** (`MENU → Network → Transfer/Remote → PC Remote: On`, `PC Remote Cnct Method: USB`) — a body in any other USB mode is not detected at all. Disable its power-save while tethered: a body that sleeps leaves the USB bus and stops being visible. On Windows the camera also needs the libusbK driver shipped with the SDK — see [the backend README](src/backends/sony/README.md).

> All native dependencies (Canon EDSDK, Nikon Remote SDK, Sony Camera Remote SDK, libgphoto2 and its camera drivers, …) are packaged inside the release archives — just download, unzip and run, nothing else to install.

> ⚠️ **One DSLR at a time.** Connecting a Nikon, a Canon and a Sony body (or several DSLRs) to the same machine simultaneously can cause cross-SDK interference: each vendor SDK probes the USB/PTP bus and may disturb the other's session, leading to slow enumeration, dropped live views, or unstable connections. For reliable operation, keep a single DSLR connected at a time. Webcams are unaffected.
